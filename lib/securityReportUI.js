...
exports.displayErrorMsg = function displayErrorMsg(aError, aMsg) {
    ...
    if (aError.category === "Content Security Policy" || aError.category === "CSP") { //csp violation
        reportUI.CSP +=  aMsg.message + "\n";
    } else if (aError.category === "Mixed Content Blocker") { // mixed-content
        reportUI.Mixed_content += aMsg.message + "\n";
    } else if (aMsg.message.indexOf("(Error") !== -1) { // ssl error code
    ...
    switch (reportUI.uiState) {
        case 1:
            ...
            break;
        case 2:
            reportUI.panelWin.document.getElementById('logTextarea').value = reportUI.CORS; 
            break;
        case 3:
    ...
    init: function(iframeWindow, aToolbox) {
       ...
       // dump("\n this._window = "+ this._window);
        ...
        reportUI.panelWin.document.documentElement.addEventListener("addon-message", function(event) {
            ...
            switch(event.detail) {
            case 1: reportUI.panelWin.document.getElementById('logTextarea').value = reportUI.CSP;
                break;
            case 2: reportUI.panelWin.document.getElementById('logTextarea').value = reportUI.CORS;
                break;
            case 3: reportUI.panelWin.document.getElementById('logTextarea').value = reportUI.Mixed_content;
                break;
...
function checkHttpHeaders(httpChannel) {
	...
	try {
		...
		// counter += 1;
		// reportUI.Sec_headers += ": Missing 'Content-Security-Policy' HTTP/S Header
		// \n";
	}
	try {
		...
		counter += 1;
		reportUI.Sec_headers += counter + ": Missing 'X-Frame-Options' Header. Your site can be framed by other websites. \n";
	}
	try {
		...
		counter += 1;
		reportUI.Sec_headers += counter + ": Missing 'Strict-Transport-Security' Header. \n";
	}
...
exports.securityReportToolUnregister = function securityReportToolUnregister() {
    ...
    removeHttpResponseObserver(); // Un-register HTTP response observer notifications    
