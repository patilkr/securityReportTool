...
let reportUI = {
		init: function(iframeWindow, aToolbox) {
			...
			// Boolean variables to store security result according to severity of an
			// error
			this.highErrors = {webPageCSP: false, webPageInsecurePwd: false};
			this.mediumErrors = {webPageSSLSelf_Signed: false, webPageSSLInvalid: false, webPageMixed_Content: false, webPageInvalidHSTS: false, webPageReportOnlyCSP:false};
			this.lowErrors = {webPageDepricatedCSP: false, webPageDepricatedReportOnlyCSP: false, webPageInlineScriptInCSP: false, webPageInlineStyleInCSP: false, webPageEvalInCSP: false, webPageDepricateInlineScriptCSP: false, webPageDepricateEvalCSP: false, webPageNonStdXhrDirCSP:false, webPageNonStdAncesDirCSP:false};
			this.logErrors = {webPageMissingHTTPS: false, webPageMissingHSTS: false, webPageMissingX_Frame_Options: false, webPageSSLUnknownIssuer: false, webPageSSLMissingChain: false, webPageSSLCaInvalid: false, webPageSSLIssuer: false, webPageSSLSignatureAlgorithmDisabled: false, webPageSSLExpiredIssuer: false, webPageSSLUntrusted: false, webPageSSLcertErrorMismatch: false, webPageSSLcertErrorExpiredNow: false, webPageSSLcertErrorNotYetValidNow: false, webPageSSLNotACACert: false, webPageSSLNotImportingUnverifiedCert: false, webPageSSLBad_Key: false, webPageSSLBad_Signature: false, webPageSSLRevoked_Certificate: false, webPageSSLUsageNotAllowed: false};
		...
		writeAReportFile: function(converter) {
			...
			try {
				...
				for (var i = 0; i < reportUI.webpageList.length; i++) {
					...
					converter.writeString(reportUI.secReport[tabs.activeTab.url][4]);
					...
					// dump("\n\n number of web pages present in the report = " +
					// reportUI.webpageList.length);
					// dump("\n Last page url = " +
					// reportUI.webpageList[reportUI.webpageList.length-1]);
...
function checkHttpHeaders(httpChannel) {
	...
	try {
		...
		reportUI.highErrors.webPageCSP = true;
		reportUI.errorCount += 1;
	}
	try {
		...
		if (cspRules) {
			...
			dump("\n\n X-CSP USED \n ");
			reportUI.lowErrors.webPageDepricatedCSP = true;
			reportUI.errorCount += 1;
		}
	} catch (e) {
	...
	try {
		...
		reportUI.logErrors.webPageMissingX_Frame_Options = true;
		reportUI.errorCount += 1;
		}
	try {
		...
		reportUI.logErrors.webPageMissingHSTS = true;
		reportUI.errorCount += 1;
	}
		...
		try {
			...
			if (contentType) {
	            ...
	            reportUI.webpageList.push(httpChannel.URI.spec);
						...
						} else { // http only web page
							 dump("\n\n Insecure web page (HTTP only)");	
							 reportUI.logErrors.webPageMissingHTTPS = true;
							 reportUI.errorCount += 1;
						}
					...
					reportUI.secReport[httpChannel.URI.spec][0] = reportUI.highErrors;
					reportUI.secReport[httpChannel.URI.spec][1] = reportUI.mediumErrors;
					reportUI.secReport[httpChannel.URI.spec][2] = reportUI.lowErrors;
					reportUI.secReport[httpChannel.URI.spec][3] = reportUI.logErrors;
...
function checkOrCreateConstStrs() {
	...
	if (!ss.storage.secReportToolStrs["reportWebPage"]) {
		ss.storage.secReportToolStrs["reportWebPage"] = " <h2>Web Page ";
	}
