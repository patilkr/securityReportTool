...
tabs.on('ready', function(evtTab) {
   ...
   // gDevTools.showToolbox(target).then(testRegister);
    ...
    gBrowser.selectedBrowser.addEventListener("load", function onLoad(evt) {
        gDevTools.showToolbox(target).then(securityReportToolRegister);
    }, true);

});

// Register Security Report Tool
function securityReportToolRegister(aToolbox) {
   toolbox = aToolbox;  
    dump("\n Toolbox panel shown to user");
 }
